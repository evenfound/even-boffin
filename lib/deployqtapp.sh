...
# Copy local dependencies libraries
...
# Create the etc directory and copy all it content.
etcdir=../etc
taretcdir=$PWD/$tardir/etc
mkdir $taretcdir
echo "Created etc directory: "$taretcdir" copying all from etc..."
...
cp $qtplatformplugins $qtplatformplugindir
echo "Copied all plugins to "$qtplatformplugindir

# Find Qt dependencies for libqxcb.so 
echo "Copying Qt dependencies for libqxcb.so..."
for dep in `ldd $qtplatformplugin | awk '{print $3}' | grep -v "("`
do
  cp $dep $libsdir
  echo "Copied dependency of "$dep" to "$libsdir
done
